    ...
    <ul aria-labelledby="list-summary" class="stack-large">
      <li v-for="item in ToDoItems" :key="item.id">
        <to-do-item :label="item.label" :done="item.done" :id="item.id"
                    @checkbox-changed="updateDoneStatus(item.id)" @item-edited="itemEdited(item.id, $event)"
...
export default {
  ...
  computed: {
    listSummary() {
      ...
      return `${numberFinishedItems} out of ${this.ToDoItems.length} ${numberFinishedItems.length <= 1? 'item' : 'items'} completed`
    }
  }
}
</script>
